项目说明：
    ...
    2、分别实现对每种操作模式下加密及解密速度的测试，要求在程序中生成 5KB 的随机测试数据，连续加密、解密 20 次，记录并报告每种模式的加密和解密的总时        间（毫秒）和 速度（KByte/秒）。
...
文件说明：
    ...
    test.py 中为实验部分代码下面做特殊说明：
        ...
        注意：本实验所有输入的数据的类型是表示16进制的字符串类型，如：23684267384FEAB90。如果输入的是其他形式的数据，请先将文件转换为二进制类型再输
             通过operations.py中的bin2string()函数转换为16进制的字符串类型。
